...
test('fetches and renders episode data from api', async () => {
    mockFetchShow.mockResolvedValueOnce(exampleResponse)

    render(<App />)

    await waitFor(() => {
        const strangerTitle = screen.queryByTestId('title')       
        expect(strangerTitle).toHaveTextContent(/stranger things/i) 
    })
